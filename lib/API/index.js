...
app.put("/menu/:id", (req, res) => {
  ...
  const sql =
    "UPDATE menu SET numero_almuerzos = ?, sopas = ?, plato_fuerte = ?, postres = ?, jugos = ?, ensaladas = ?, extras = ?, fecha = ? WHERE id = ?";
  db.query(
    sql,
    [numero_almuerzos, sopas, plato_fuerte, postres, jugos, ensaladas, extras, fecha, id],
    (err, result) => {
      if (err) throw err;
